...
export default function FlowView() {
  ...
  useEffect(() => {
    ...
    Promise.all([api.get('/api/auth/me'), api.get(`/api/docs/${id}`)])
      .then(([userRes, docRes]) => {
        ...
        const foundFlow = (docRes.data.doc.flows || []).find((f) => f.id === flowId)
        if (!foundFlow) {
          navigate(`/project/${id}`)
          return
        }
  ...
  const handleAddLink = async () => {
    ...
    if (!url) return
    try {
      const { data } = await api.post(`/api/docs/${id}/flows/${flowId}/files`, {
        type: 'link',
        name: url,
        url,
      })
      setFlow((prev) => ({ ...prev, files: [...(prev.files || []), data.file] }))
      setLinkUrl('')
  ...
  const handleAddFile = async (file) => {
    try {
      ...
      const { data } = await api.post(`/api/docs/${id}/flows/${flowId}/files`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      })
      setFlow((prev) => ({ ...prev, files: [...(prev.files || []), data.file] }))
    } catch (err) {
      console.error('Failed to upload file to flow:', err)
    }
  }
        ...
        <div className="flex-1 overflow-y-auto px-[39px] pb-12 pt-8">
          ...
          <div className="mb-6 flex items-center gap-2">
            <button
              ...
              className="cursor-pointer text-[15px] font-[590] text-[#8d8d97] transition-colors hover:text-white"
            >
              Flow Repository
            </button>
          ...
          {/* Add Files button + dropdown */}
          <div className="relative">
            <button
              onClick={() => setShowAddMenu((v) => !v)}
              className="flex h-[39px] cursor-pointer items-center gap-2 rounded-[9px] border border-[rgba(178,197,242,0.3)] bg-transparent px-4 text-[13px] text-[#b2c5f2] transition-colors hover:border-[#b2c5f2]"
            >
            ...
            {/* Simplified dropdown — file + link only */}
            {showAddMenu && (
              <>
                ...
                <div className="absolute z-50 mt-2 w-[260px] overflow-hidden rounded-[13px] border border-[rgba(150,150,165,0.3)] bg-[#0e0e1a] shadow-[0px_20px_60px_rgba(0,0,0,0.6)]">
                  <button
                    ...
                    className="flex w-full cursor-pointer items-center justify-between px-5 py-3 text-[15px] text-white transition-colors hover:bg-[rgba(255,255,255,0.06)]"
                  >
          ...
          {showLinkInput && (
            <div className="mt-4 flex items-center gap-3">
              ...
              <button
                ...
                className="cursor-pointer text-[13px] text-[#8d8d97] hover:text-white"
              >
                Cancel
              </button>
            </div>
