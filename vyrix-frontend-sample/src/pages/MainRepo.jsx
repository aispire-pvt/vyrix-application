...
// Project Based Repo (Phase 10) — every project + every uploaded file in one place.
export default function MainRepo() {
  ...
  // null = show files from all projects; otherwise filter to this project.
  const [selectedProjectId, setSelectedProjectId] = useState(null)

  useEffect(() => {
    let active = true
    Promise.all([api.get('/api/auth/me'), api.get('/api/docs')])
  ...
  const sortedDocs = [...docs].sort((a, b) => {
    ...
    if (key === 'last_edited') return new Date(b.updatedAt) - new Date(a.updatedAt)
    return new Date(b.createdAt) - new Date(a.createdAt)
  })

  // Uploaded files (attachments + flow files) for a single project.
  const filesOf = (doc) => [
    ...(doc?.attachments || []),
    ...(doc?.flows || []).flatMap((f) => f.files || []),
  ]

  const selectedDoc = selectedProjectId ? docs.find((d) => d._id === selectedProjectId) : null

  // Show all projects' files by default, or just the selected project's.
  const shownFiles = (selectedDoc ? filesOf(selectedDoc) : docs.flatMap(filesOf)).sort(
        ...
        <div className="dark-scroll flex-1 overflow-y-auto px-[39px] pb-12 pt-8">
          ...
          <div className="mb-6 flex items-start justify-between">
            ...
            <button
              ...
              className="flex h-[74px] cursor-pointer items-center gap-3 rounded-[20px] bg-[rgba(12,12,24,0.27)] px-5 shadow-[0px_4px_7.6px_-3px_rgba(255,255,255,0.24)] transition-colors hover:bg-[rgba(12,12,24,0.5)]"
            >
              <svg width="22" height="18" viewBox="0 0 22 18" fill="none" className="opacity-80">
                <path d="M1 3a2 2 0 012-2h4l2 2h10a2 2 0 012 2v10a2 2 0 01-2 2H3a2 2 0 01-2-2V3z" stroke="#d5d5d5" strokeWidth="1.5" />
              </svg>
              <span className="text-[20px] font-bold text-[#d5d5d5]">Go to your projects</span>
            </button>
          </div>

          {/* Sort By + Show Folders */}
          <div className="mb-6 flex items-center gap-4">
            ...
            <button
              ...
              className="flex cursor-pointer items-center gap-2 rounded-[15px] border border-[#a6a6a6] bg-[rgba(14,16,34,0.62)] px-4 py-[3px] transition-colors hover:border-[#b2c5f2]"
            >
              <span className="text-[14px] text-[#d5d5d5]">Show Folders</span>
              <div
                className={`flex h-[17px] w-[17px] items-center justify-center rounded-[4px] border-2 ${
                  showFolders ? 'border-[#b2c5f2] bg-[#b2c5f2]' : 'border-[#5a5a6e]'
                }`}
              >
                {showFolders && (
                  <svg width="9" height="9" viewBox="0 0 9 9" fill="none">
                    <path d="M1.5 4.5l2 2 4-4" stroke="black" strokeWidth="1.5" strokeLinecap="round" />
                  </svg>
                )}
              </div>
            </button>
          ...
          {/* Projects — click a card to filter the files below to that project */}
          ...
          {docs.length === 0 ? (
            ...
            <div className="mb-8 flex flex-wrap gap-[18px]">
              {sortedDocs.map((doc) => {
                const selected = doc._id === selectedProjectId
                return (
                  <div
                    key={doc._id}
                    onClick={() => setSelectedProjectId(selected ? null : doc._id)}
                    className="group flex cursor-pointer flex-col gap-2"
                  >
                    <div
                      className={`relative h-[164px] w-[266px] overflow-hidden rounded-[20px] shadow-[0px_4px_4.5px_2px_rgba(0,0,0,0.25)] transition-all group-hover:-translate-y-0.5 ${
                        selected ? 'ring-2 ring-[#b2c5f2] ring-offset-2 ring-offset-black' : ''
                      }`}
                    >
                      <img
                      ...
                      {selected && (
                        <div className="absolute right-3 top-3 flex h-[26px] w-[26px] items-center justify-center rounded-full bg-[#b2c5f2] shadow-md">
                          <svg width="13" height="13" viewBox="0 0 14 14" fill="none">
                            <path d="M2.5 7.5l3 3 6-6" stroke="black" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                          </svg>
                        </div>
                      )}
                    </div>
                    <p
                      className={`max-w-[266px] truncate text-[16px] font-[590] ${
          ...
          {/* Uploaded files — all projects, or just the selected one */}
          <div className="mb-6 flex items-center gap-4">
            ...
            {selectedDoc && (
              <button
                ...
                className="flex cursor-pointer items-center gap-2 rounded-[15px] border border-[#a6a6a6] bg-[rgba(14,16,34,0.62)] px-4 py-1 text-[13px] text-[#d5d5d5] transition-colors hover:border-[#b2c5f2]"
              >
          ...
          {shownFiles.length === 0 ? (
            <div className="flex h-[120px] items-center justify-center rounded-[20px] border border-dashed border-[rgba(178,197,242,0.15)]">
              <p className="text-[14px] text-[#4a4a5a]">
                {selectedDoc
                  ? 'No files uploaded in this project yet'
                  : 'No uploaded files yet — upload files inside your projects'}
              </p>
            </div>
          ) : (
            <div className="flex flex-wrap gap-[24px]">
              {shownFiles.map((file, i) => {
                ...
                return file.url ? (
                  <a key={file.id || i} href={file.url} target="_blank" rel="noreferrer">
                    {card}
                  </a>
                ) : (
