// Slide-in To-Do panel (toggled by the navbar "My To Do" button).
// Working todo backed by the API (per-user, persisted in the DB) so tasks are
// there whenever the user comes back: add a task, click to complete
// (strike-through), delete it.
...
export default function TodoPanel({ isOpen, onClose, firstName }) {
  ...
  // Load the user's todos from the DB whenever the panel opens.
  useEffect(() => {
    if (!isOpen) return
    let active = true
    setLoading(true)
    api
      .get('/api/todos')
      .then((res) => {
        if (active) setTasks(res.data.todos)
      })
  ...
  const toggleTask = async (task) => {
    ...
    try {
      await api.patch(`/api/todos/${task._id}`, { done: !task.done })
    } catch (err) {
    ...
    <div className="absolute right-0 top-0 z-50 mr-4 mt-[93px] flex h-[543px] w-[415px] flex-col gap-4 rounded-[18px] bg-[#0e0e1a] p-[25px] shadow-2xl">
      ...
      <div className="flex items-center gap-3">
        <div className="flex h-[41px] w-[41px] flex-shrink-0 items-center justify-center rounded-full border-2 border-[#b2c5f2]">
          <span className="text-[11px] font-bold text-[#b2c5f2]">
            {doneCount}/{total}
          </span>
        </div>
        <p className="text-[14px] text-[#d5d5d5]">
          {total === 0
            ? `Add your first task, ${firstName || 'there'}!`
      ...
      <div className="flex h-[35px] w-full items-center gap-2 rounded-[11px] border border-[rgba(178,197,242,0.2)] bg-[rgba(178,197,242,0.08)] px-3">
        <button type="button" onClick={addTask} className="flex cursor-pointer items-center" aria-label="Add task">
          <PlusIcon />
        </button>
        <input
          ref={inputRef}
      ...
      <div className="flex flex-1 flex-col gap-1 overflow-y-auto">
        {loading ? (
          ...
          <>
            ...
            {ongoing.map((task) => (
              <div key={task._id} className="group flex items-center gap-3 py-1">
                <button
                  type="button"
                  onClick={() => toggleTask(task)}
                  className="h-[17px] w-[17px] flex-shrink-0 cursor-pointer rounded-[3px] border border-[rgba(178,197,242,0.4)] bg-transparent transition-colors hover:border-[#b2c5f2]"
                  aria-label="Complete task"
                />
                <button
                  type="button"
                  onClick={() => toggleTask(task)}
                  className="flex-1 cursor-pointer truncate text-left text-[14px] text-[#d5d5d5]"
                >
                  {task.text}
                </button>
                <button
                  ...
                  className="flex-shrink-0 cursor-pointer text-[#5a5a6a] opacity-0 transition-opacity hover:text-[#b80407] group-hover:opacity-100"
            ...
            {completed.map((task) => (
              <div key={task._id} className="group flex items-center gap-3 py-1">
                <button
                  type="button"
                  onClick={() => toggleTask(task)}
                  className="flex h-[17px] w-[17px] flex-shrink-0 cursor-pointer items-center justify-center rounded-[3px] border border-[#b2c5f2] bg-[#b2c5f2] text-[10px] font-bold text-black"
                ...
                <button
                  type="button"
                  onClick={() => toggleTask(task)}
                  className="flex-1 cursor-pointer truncate text-left text-[14px] text-[#8d8d97] line-through"
                >
                  {task.text}
                </button>
                <button
