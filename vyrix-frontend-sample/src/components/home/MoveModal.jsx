...
export default function MoveModal({ isOpen, onClose, docId, docTitle, currentFolder, folders = [], onMoved }) {
  ...
  const filtered = localFolders.filter((f) =>
  ...
  const handleMove = async () => {
    ...
    setMoving(true)
    try {
      await api.patch(`/api/docs/${docId}/move`, { folderId: selected })
  ...
  const handleCreateFolder = async () => {
    ...
    try {
      ...
      setLocalFolders((prev) => [data.folder, ...prev])
      setSelected(data.folder._id)
    } catch (err) {
    ...
    >
      <div className="w-[413px] overflow-hidden rounded-[22px] border border-[rgba(140,140,140,0.5)] bg-[rgba(12,12,24,0.95)] shadow-[0px_40px_100px_rgba(0,0,0,0.6)]">
        ...
        <div className="relative border-b border-[rgba(150,150,165,0.16)] px-[27px] pb-[19px] pt-[25px]">
          ...
          <p className="text-[18.6px] font-bold text-white">
            Move <span className="text-[#b2c5f2]">{docTitle}</span>
          </p>
          <div className="mt-[11px] flex items-center gap-2">
            <svg width="15" height="15" viewBox="0 0 15 15" fill="none">
              <path d="M1.5 4.5h12M1.5 7.5h9M1.5 10.5h6" stroke="#8d8d97" strokeWidth="1.5" strokeLinecap="round" />
            </svg>
            <span className="text-[12.7px] text-[#8d8d97]">Currently in</span>
            <span className="text-[12.7px] text-[#c7c7c7]">
              {currentFolder ? currentFolder.name : 'Recent · No folder'}
            </span>
          </div>
        </div>
        ...
        <div className="mx-[27px] mt-[12px] flex items-center gap-3 rounded-[13px] border border-[#dcdcdc] px-[16px] py-[13px]">
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
            <circle cx="7" cy="7" r="5" stroke="#7e7e8c" strokeWidth="1.5" />
            <path d="M11 11l3 3" stroke="#7e7e8c" strokeWidth="1.5" strokeLinecap="round" />
          </svg>
          <input
            ref={searchRef}
            type="text"
            ...
            className="flex-1 bg-transparent text-[13.7px] text-[#e7e7e7] outline-none placeholder:text-[#7e7e8c]"
          />
        </div>
        ...
        <div className="mt-[8px] max-h-[294px] overflow-y-auto px-[17px] py-[8px]">
          {filtered.map((folder) => (
            <div
              ...
              className={`flex cursor-pointer items-center gap-[14.7px] rounded-[13px] px-[14.7px] py-[13.72px] transition-colors ${
                selected === folder._id
                  ? 'bg-[rgba(178,197,242,0.08)]'
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-[12px] text-[#8d8d97]">
                  {folder.docCount ?? 0} projects · {relativeTime(folder.updatedAt || folder.createdAt)}
                </p>
              </div>

              <div
                className={`flex h-[21.56px] w-[21.56px] flex-shrink-0 items-center justify-center rounded-full border-2 transition-colors ${
                  selected === folder._id
                    ? 'border-[#b2c5f2] bg-[#b2c5f2]'
                    : 'border-[#5a5a6e]'
                }`}
              >
                {selected === folder._id && <div className="h-[8px] w-[8px] rounded-full bg-black" />}
              </div>
            </div>
          ))}
          ...
          {showNewInput ? (
            ...
            <div
              ...
              className="flex cursor-pointer items-center gap-[14.7px] rounded-[13px] px-[14.7px] py-[13.72px] transition-colors hover:bg-[rgba(255,255,255,0.04)]"
            >
              <div className="flex h-[33px] w-[41px] flex-shrink-0 items-center justify-center rounded-[9px] border border-dashed border-[rgba(178,197,242,0.5)]">
        ...
        <div className="flex items-center justify-end gap-3 border-t border-[rgba(150,150,165,0.16)] px-[27px] py-[19px]">
          <button
            onClick={onClose}
            className="cursor-pointer rounded-[13px] border border-[rgba(140,140,140,0.5)] px-[24px] py-[12px] text-[14.2px] font-bold text-[#d5d5d5] transition-colors hover:bg-[rgba(255,255,255,0.05)]"
          >
            Cancel
          </button>
          <button
            ...
            className={`rounded-[13px] px-[24px] py-[12px] text-[14.2px] font-bold text-black transition-all ${
              selected && !moving
                ? 'cursor-pointer bg-[#b2c5f2] hover:bg-[#c5d4f5]'
                : 'cursor-not-allowed bg-[#b2c5f2] opacity-40'
            }`}
          >
