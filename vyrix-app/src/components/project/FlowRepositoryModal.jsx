...
export default function FlowRepositoryModal({ isOpen, onClose, projectId, flows = [], onFlowsChange }) {
  ...
  const handleDeleteFlow = async (flowId) => {
    ...
    setLocalFlows(next)
    try {
      await flowsApi.remove(projectId, flowId)
    } catch (err) {
  ...
  const handleAddLink = async () => {
    ...
    if (!url) return
    try {
      const file = await flowsApi.addLink(projectId, selectedFlowId, { type: 'link', name: url, url })
      updateFlowFiles(file)
      setLinkUrl('')
      setShowLinkInput(false)
    } catch (err) {
  ...
  const handleAddFile = async (file) => {
    try {
      ...
      const added = await flowsApi.addFile(projectId, selectedFlowId, sourcePath, { name: file.name })
      updateFlowFiles(added)
    } catch (err) {
